# Question -: A picnic to a famous museum is being planned in a school for class VI. When they reached the spot, the students started quarreling among themselves in the queue. So the teacher came up with an idea of “good string” which is explained below.
...
for letter in name:
    ...
    for char in good_str:
        current_dist = abs(ord(char) - ord(letter))
        if current_dist < min_dist:
            min_dist = current_dist
            min_char = char
            equidistant_chars = [char]
        elif current_dist == min_dist:
            equidistant_chars.append(char)

    if len(equidistant_chars) > 1:
        closest_char = min(equidistant_chars, key=lambda x: abs(ord(x) - ord(prev_good_letter)))
        distance += abs(ord(closest_char) - ord(prev_good_letter))
        prev_good_letter = closest_char
    else:
        distance += min_dist
        prev_good_letter = min_char
